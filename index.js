const apiConfig = {
  ...
  server.get('/itens', async (req, res) => {
    const client = new Client(databaseConfig)
    await client.connect()
    const result = await client.query({
      text: 'select * from itens;'
    })
    await client.end()
    res.send(result.rows)
  })
  server.get('/itens/:itemid', async (req, res) => {
    const client = new Client(databaseConfig)
    await client.connect()
    const result = await client.query({
      text: 'select * from itens where id = $1;',
  ...
  server.post('/itens', async (req, res) => {
    const client = new Client(databaseConfig)
  ...
  server.put('/itens/:itemid', async (req, res) => {
    const client = new Client(databaseConfig)
    await client.connect()
    const result = await client.query({
  ...
  server.delete('/itens/:itemid', async (req, res) => {
    const client = new Client(databaseConfig)
    await client.connect()
    const result = await client.query({
